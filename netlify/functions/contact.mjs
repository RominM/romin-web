...
export async function handler(event) {
  ...
  try {
    ...
    await resend.emails.send({
      from: 'Romin <onboarding@resend.dev>', // à remplacer par ton email une fois le domaine vérifié sur Resend
      to: email,
    ...
    // 2. Notification à Romin avec tous les détails
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 560px; margin: 0 auto; color: #1c1729;">
          ...
          <div style="margin-top: 1.5rem; padding: 1rem; background: #f7f4ff; border-radius: 8px; color: #1c1729; line-height: 1.7;">
            ${message}
          </div>
          <p style="margin-top: 1.5rem;">
            <a href="mailto:${email}" style="background: #7b69f8; color: #fff; padding: 0.6rem 1.4rem; border-radius: 100px; text-decoration: none; font-weight: 600;">
              Répondre à ${name}
            </a>
          </p>
        </div>
